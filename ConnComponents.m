...
if(length(xyz(:,1))>20000)
    p=pointCloud(xyz);
    paux=pcdownsample(p,'gridAverage',mst_grid*1.5);  
    xyzaux=paux.Location;
    minsize=counts_size*length(xyzaux)/length(xyz)*1.2;%new min size 

elseif length(xyz(:,1))>10000  
    p=pointCloud(xyz);
